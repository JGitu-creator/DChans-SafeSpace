...
const LOCK_SCREEN_BIKES = [
  ...
  'https://images.unsplash.com/photo-1449491023939-02c9ab99407d?auto=format&fit=crop&q=80&w=2070',
];
...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen w-full overflow-hidden font-sans text-zinc-900 bg-black">
      <div className="absolute inset-0 z-0 overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div key={isLocked ? `lock-${lockImageIndex}` : currentMood.terrain} initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 2 }} className="absolute inset-0">
            <img src={isLocked ? LOCK_SCREEN_BIKES[lockImageIndex] : backgroundImages[currentMood.terrain]} className="w-full h-full object-cover opacity-60" alt="Background" />
      ...
      <AnimatePresence>
        {isLocked ? (
          <motion.div key="lock-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="relative z-50 flex min-h-screen flex-col items-center justify-center p-8 text-white backdrop-blur-sm">
            ...
            <motion.div initial={{ y: 20 }} animate={{ y: 0 }} className="flex flex-col items-center gap-12 w-full max-w-sm text-center">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-white/40 text-xs font-black uppercase tracking-[0.4em] leading-none text-purple-400">The Highway of Holiness</p>
              </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row min-h-screen h-full overflow-hidden">
            <header className="md:hidden flex items-center justify-between p-6 bg-black/60 backdrop-blur-xl border-b border-white/5 text-white">
            ...
            <aside className={`fixed md:relative inset-y-0 left-0 z-40 w-72 md:w-80 p-8 flex flex-col gap-6 bg-black/95 md:bg-black/40 backdrop-blur-3xl transition-transform duration-500 ease-in-out border-r border-white/5 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col gap-8 pr-2">
                ...
                <nav className="flex flex-col gap-1">
                  ...
                  <button onClick={() => { setActiveView('affirmation'); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all ${activeView === 'affirmation' ? 'bg-purple-600 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}><MapPin size={16} /><span className="text-sm font-bold uppercase tracking-tight">The Path</span></button>
                  <button onClick={() => { setActiveView('chat'); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all ${activeView === 'chat' ? 'bg-purple-600 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}><Sparkles size={16} /><span className="text-sm font-bold uppercase tracking-tight text-emerald-400">Selig Chat</span></button>
                  <button onClick={() => { setActiveView('journal'); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all ${activeView === 'journal' ? 'bg-purple-600 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}><MessageSquare size={16} /><span className="text-sm font-bold uppercase tracking-tight">Road Log</span></button>
                  <button onClick={() => { setActiveView('trail'); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all ${activeView === 'trail' ? 'bg-purple-600 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}><MapPin size={16} /><span className="text-sm font-bold uppercase tracking-tight">The Trail</span></button>
                  <button onClick={() => { setActiveView('basket'); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all ${activeView === 'basket' ? 'bg-purple-600 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}><Book size={16} /><span className="text-sm font-bold uppercase tracking-tight">Gleaning</span></button>
                  <button onClick={() => { setActiveView('pitstop'); setIsSidebarOpen(false); }} className={`flex items-center gap-4 px-4 py-3 rounded-2xl transition-all ${activeView === 'pitstop' ? 'bg-purple-600 text-white font-bold' : 'text-white/50 hover:bg-white/5 hover:text-white'}`}><Zap size={16} /><span className="text-sm font-bold uppercase tracking-tight text-sky-400">Pit Stop</span></button>
                </nav>
              </div>
            ...
            <section className="flex-1 flex flex-col items-center justify-start md:justify-center p-6 md:p-12 overflow-y-auto no-scrollbar relative bg-black/20">
              <AnimatePresence mode="wait">
                {rescueMode && <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-2xl bg-blue-500/20 border border-blue-500/30 p-6 rounded-3xl mb-8 flex items-center gap-6"><div className="p-4 bg-sky-500 rounded-2xl text-white"><Zap size={32} /></div><div><h4 className="text-lg font-black italic text-white uppercase tracking-tighter">Search & Rescue</h4><p className="text-sm text-blue-100/70 leading-relaxed font-medium">¡Hola! I went on a ride to find you, Hadassah. Selig is here now.</p></div><button onClick={() => setRescueMode(false)} className="ml-auto text-white/20 hover:text-white"><X /></button></motion.div>}
                    ...
                    <div className="relative group">
                      ...
                      <div className="relative flex items-center gap-6 bg-black/60 backdrop-blur-2xl rounded-[3rem] p-6 border border-white/10 shadow-2xl">
                        ...
                        <div className="flex-1">
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-purple-400 mb-1">Equipped Gear</p>
                          <h3 className="text-3xl font-black uppercase italic tracking-tighter text-white leading-none mb-2">{currentMood.bike} Mode</h3>
                          <div className="flex items-center gap-2 text-white/40"><MapPin size={12} className="text-sky-400" /><span className="text-[10px] font-bold uppercase tracking-widest">{currentMood.terrain} Terrain</span></div>
                        </div>
                        <div className="hidden md:flex flex-col items-end gap-2 pr-4"><div className="p-3 bg-white/5 rounded-2xl border border-white/10 text-white/60"><Zap size={24} /></div><span className="text-[9px] font-black uppercase tracking-widest text-white/20 italic text-sky-400">Selah Engine v3.0</span></div>
                      </div>
                    </div>
