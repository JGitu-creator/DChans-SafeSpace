    ...
    <main className="relative min-h-screen w-full font-sans text-zinc-900 bg-black overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0">
        <AnimatePresence mode="wait">
          <motion.div 
            key={isLocked ? `lock-${lockImageIndex}` : currentRoute.terrain} 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
          ...
          >
            <img src={isLocked ? LOCK_SCREEN_BIKES[lockImageIndex] : backgroundImages[currentRoute.terrain]} className="w-full h-full object-cover opacity-60" alt="Background" />
            ...
            <div 
              ...
              style={{ 
                background: `radial-gradient(circle at 20% 20%, ${currentRoute.accentColor}33 0%, transparent 50%), radial-gradient(circle at 80% 80%, ${currentRoute.secondaryColor}33 0%, transparent 50%)` 
              }} 
            />
          </motion.div>
        </AnimatePresence>
      ...
      <AnimatePresence>
        {isLocked ? (
          /* Sleek Minimalist Lock Screen */
          <motion.div key="lock-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.05 }} className="relative z-50 flex min-h-screen flex-col items-center justify-center p-8 text-white backdrop-blur-sm">
            ...
            <motion.div initial={{ y: 20 }} animate={{ y: 0 }} className="flex flex-col items-center gap-12 w-full max-w-sm text-center">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.6em] leading-none mt-4">The Highway of Holiness</p>
              </div>
          ...
          /* Main Luminous Scrapbook Shell */
          <div className="relative z-10 flex flex-col min-h-screen">
            {/* Navigation Bracelet (Desktop/Mobile) */}
            <CharmBracelet 
              activeView={activeView} 
              onViewChange={setActiveView} 
              accentColor={currentRoute.accentColor} 
            />

            <div className="flex flex-col md:flex-row flex-1">
              {/* Left Sidebar - Routes (Optional/Collapsible) */}
              <aside className="hidden lg:flex w-80 p-8 flex-col gap-8 bg-black/20 backdrop-blur-3xl border-r border-white/5 overflow-y-auto no-scrollbar h-[calc(100vh-100px)] sticky top-[100px]">
                <MoodSelector 
                  currentMoodId={currentRoute.id} 
                  onSelectMood={setCurrentRoute} 
                />
                ...
                <div className="mt-auto pt-6 border-t border-white/5 flex flex-col gap-2">
                  ...
                  <button onClick={() => setIsLocked(true)} className="flex items-center justify-center gap-3 py-4 rounded-2xl bg-red-500/10 text-red-500 active:scale-95 shadow-xl"><LogOut size={16} /><span className="text-[10px] font-black uppercase tracking-widest">Lock Sanctuary</span></button>
                </div>
              </aside>
                      ...
                      <div className="relative group">
                        ...
                        <div className="relative flex items-center gap-6 bg-black/60 backdrop-blur-3xl rounded-[3rem] p-8 border border-white/10 shadow-2xl overflow-hidden">
                          ...
                          <div className="flex-1">
                            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-sky-400 mb-2">Equipped for the journey</p>
