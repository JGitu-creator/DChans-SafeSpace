...
const getDailyAffirmation = (routeId: string): Affirmation => {
  ...
  const affirmations: Record<number, Affirmation> = {
    0: { proverbHook: "A quiet heart is a royal sanctuary.", growthWord: { word: "Serenity", definition: "The state of being calm, peaceful, and untroubled." }, spanishPhrase: { phrase: "Dios es mi refugio.", translation: "God is my refuge." }, deepExegesis: "Like a calm lake reflecting the mountain, let your soul reflect the King's peace today.", bibleVerse: "Psalm 46:10" },
    1: { proverbHook: "A journey of a thousand miles begins with a single turn of the key.", growthWord: { word: "Resilience", definition: "The capacity to recover quickly from difficulties; toughness." }, spanishPhrase: { phrase: "Eres amada y elegida.", translation: "You are loved and chosen." }, deepExegesis: "In the story of Ruth, she faced a crossroads. She chose the road to Bethlehem. Today, Hadassah, your loyalty to your true self and your Father is your greatest 'gear'.", bibleVerse: "Ruth 1:16" },
    2: { proverbHook: "The hardest roads often lead to the most beautiful vistas.", growthWord: { word: "Persistence", definition: "Firm or obstinate continuance in a course of action in spite of difficulty." }, spanishPhrase: { phrase: "Sigue adelante con fe.", translation: "Go forward with faith." }, deepExegesis: "The road may be steep, but your gear is divine. Every climb is a lesson in His strength.", bibleVerse: "Isaiah 40:31" },
    3: { proverbHook: "True beauty is found in the brushstrokes of grace.", growthWord: { word: "Elegance", definition: "The quality of being graceful and stylish in appearance or manner." }, spanishPhrase: { phrase: "Gracia sobre gracia.", translation: "Grace upon grace." }, deepExegesis: "You are the King's masterpiece. Every detail of your life is being painted with intentional love.", bibleVerse: "Ephesians 2:10" },
    4: { proverbHook: "Strength isn't just about speed; it's about staying the course.", growthWord: { word: "Fortitude", definition: "Courage in pain or adversity." }, spanishPhrase: { phrase: "Fuerte y valiente.", translation: "Strong and courageous." }, deepExegesis: "Like Esther entering the court, your courage is your crown. Ride with the authority given to you.", bibleVerse: "Joshua 1:9" },
    5: { proverbHook: "Rest is not idleness; it is the rhythm of the soul.", growthWord: { word: "Restoration", definition: "The action of returning something to a former owner, place, or condition." }, spanishPhrase: { phrase: "Descansa en su amor.", translation: "Rest in His love." }, deepExegesis: "The King invites you to a banquet of rest. Lay down the heavy gear and just be His daughter.", bibleVerse: "Matthew 11:28" },
    6: { proverbHook: "Light always finds a way through the thickest forest.", growthWord: { word: "Luminosity", definition: "Luminous quality." }, spanishPhrase: { phrase: "Luz en mi camino.", translation: "Light on my path." }, deepExegesis: "Your light shines brightest in the dark valleys. Do not fear the shadows; the Sun of Righteousness is rising.", bibleVerse: "Psalm 119:105" }
  };
  ...
  useEffect(() => {
    if (!isLocked) {
      ...
      setSettings(prev => {
        ...
        if (hoursSince > 48 && !rescueMode) { setRescueMode(true); }
  ...
  const transformStruggle = async () => {
    ...
    try {
      const response = await fetch('/api/selig', {
        ...
        body: JSON.stringify({ struggle: struggleInput, settings })
      });
      ...
      <AnimatePresence>
        ...
        {isLocked ? (
          <motion.div key="lock-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.05 }} className="relative z-50 flex min-h-screen flex-col items-center justify-center p-6 text-white overflow-hidden">
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="flex flex-col items-center gap-6 w-full max-w-lg text-center relative z-10 -mt-12">
              <div className="flex flex-col items-center gap-0">
                <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 4 }} className="text-amber-400 mb-2">
                  <Crown size={32} strokeWidth={1.5} className="drop-shadow-[0_0_15px_rgba(251,191,36,0.5)]" />
                </motion.div>
                <h1 className="flex flex-col items-center gap-0">
                  <span className="text-4xl md:text-6xl font-bodoni font-black italic tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-b from-purple-400 via-purple-600 to-purple-900 drop-shadow-xl leading-none">DCHAN&apos;S</span>
                  <span className="text-6xl md:text-8xl font-normal text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-600 font-pinyon block drop-shadow-[0_0_20px_rgba(251,191,36,0.2)]">SafeSpace</span>
                </h1>
                <p className="text-[9px] font-marcellus tracking-[0.8em] text-white/30 uppercase mt-2 italic">Chantal Hadassah</p>
              </div>

              <div className="w-full flex flex-col gap-6 max-w-[280px] mt-6">
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="relative w-full bg-black/60 border border-white/10 rounded-2xl px-6 py-4 text-center text-3xl tracking-[0.8em] focus:outline-none focus:border-white/30 backdrop-blur-md text-white placeholder:text-white/10 placeholder:tracking-widest placeholder:text-[8px] transition-all font-marcellus" 
                    onKeyDown={(e) => e.key === 'Enter' && handleUnlock()} 
                  />
                </div>
                
                <button onClick={handleUnlock} className="group relative py-4 overflow-hidden transition-all active:scale-95">
                  ...
                  <div className="relative flex items-center justify-center gap-4">
                    ...
                    <span className="text-[8px] font-marcellus uppercase tracking-[0.5em] text-white group-hover:text-amber-400 transition-colors">Enter Sanctuary</span>
                    <div className="w-6 h-px bg-white/20 group-hover:w-10 transition-all" />
                  </div>
                </button>
              </div>
              ...
              <div className="flex flex-col items-center gap-3 mt-4">
                 ...
                 <div className="flex items-center gap-2 text-white/20">
                   ...
                   <p className="text-[6px] font-black uppercase tracking-[0.5em] italic">For Her Heart Alone</p>
                 </div>
              </div>
            </motion.div>
          </motion.div>
          ...
          <div className="relative z-10 flex flex-col min-h-screen pt-4 pb-20 px-0 md:px-8">
            <div className="flex justify-between items-center px-6 md:px-4 mb-4">
               ...
               <button onClick={() => setIsLocked(true)} className="group flex items-center gap-2 bg-white/5 hover:bg-red-500/10 px-4 py-2 rounded-xl border border-white/5 hover:border-red-500/20 transition-all text-white/40 hover:text-red-400">
                    ...
                    {activeView === 'affirmation' && (
                      <motion.div key="affirmation-view" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col gap-10">
                        <div className="flex items-center gap-4">
                          <div className="relative group"><div className="absolute -inset-1 bg-gradient-to-tr from-purple-500 to-sky-400 rounded-full blur opacity-20"></div><span className="relative text-5xl filter drop-shadow-md cursor-help transition-transform hover:scale-110 block">{currentRoute.sticker}</span></div>
                          <div className="flex flex-col">
                            <div className="bg-white/80 backdrop-blur-sm px-4 py-2 rounded-2xl rounded-tl-none border border-black/5 shadow-sm relative"><p className="text-zinc-800 text-sm font-medium italic">{(() => { const hour = new Date().getHours(); if (hour < 11) return "The road is fresh, Hadassah. Your gear is ready."; if (hour < 17) return "Take a pit stop, mi hermana. His grace is sufficient."; if (hour < 21) return "Ride towards the sunset of His mercy."; return "Park the bike in His peace. You are well-guarded."; })()}</p></div>
